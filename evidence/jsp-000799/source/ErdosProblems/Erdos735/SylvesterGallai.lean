...
Pure kernel: no unfinished proof declarations, and every theorem in this file depends only on
`[propext, Classical.choice, Quot.sound]`, as measured by a separate kernel audit.
...
development exists (github.com/Happyves/Master_Thesis, but is incomplete). In Lean 4,
...
affine lines) is only sketched there in a commented-out incomplete block. To our
