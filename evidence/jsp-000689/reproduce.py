"""Reproduce JSP-000689 / Erdos 833 using explicitly supplied pinned dependencies.
...
SOURCE_HASH = '5cad260bdd589d4be977765c3934f8389395c6e6b480efee4dd8e325a7bf7cdb'
...
def main():
    ...
    if sha(ROOT / 'Erdos833.lean') != SOURCE_HASH:
    ...
    if args.stage in ['compile', 'all']:
        commands += [('compile-source', [str(lean), '-j1', '-M4096', '-o', 'Erdos833.olean', 'Erdos833.lean']),
                     ('compile-audit', [str(lean), '-j1', '-M4096', '-o', 'Audit833.olean', 'Audit833.lean'])]
    if args.stage in ['checker', 'all']:
        commands += [('leanchecker', [str(checker), '--verbose', 'Erdos833'])]
